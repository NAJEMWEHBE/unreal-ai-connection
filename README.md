...
107 tools total. Zero pixel-clicking. ~50ms round-trip.
...
[![Tests](https://img.shields.io/badge/pytest-529_passing-success?logo=pytest&logoColor=white)](tests/)
...
**Native C++ handlers — not Python Remote Execution.** ~50 ms round-trips across 107 tools · 529 tests · MIT · works with Claude Code, Cursor, Cline, Codex, Gemini, Continue, Windsurf & Zed.
...
| **Tools** | **105 live** — 72 native C++ handlers (one MCP method per `Handler_*.cpp`) plus 35 bridge-side synthetic tools (Python-only composition over existing handlers; never crosses the TCP wire as a dedicated round-trip). See [`docs/TOOLS.md`](docs/TOOLS.md) for the per-tool reference. |
...
| **Bridge tests** | 529 pytest cases, ~99% coverage |
