...
- **`bulk_spawn_actors` synthetic tool** (pending PR). Completes the actor-batch lifecycle alongside Wave C's `bulk_focus_actors`, `bulk_screenshot_actors`, and `bulk_set_actor_property`. Composes `spawn_actor` bridge-side; identical partial-failure semantics (`continue_on_error` default `true`, `halted_at_index` on halt). Max 100 spawns per call; each entry carries its own `class_path` (required), `location`, `rotation`, `label`, and `properties`. Tool count 104 → **105** (71 C++ + 34 synthetic). 10 new pytest cases (Wave D label, happy path, `continue_on_error` true/false, missing-`spawns`, non-object entry, missing-`class_path`, too-many, optional-field forwarding, invalid-`location`-type). Drift sweep, manifest-sync, and conftest counts bumped in the same commit.
