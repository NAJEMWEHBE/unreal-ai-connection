...
animated demo loop embedded in the README:
  ...
  1. creates/loads a DEDICATED throwaway scratch level
     (/Game/_McpScratch/DemoLevel) so the build never dirties the
     user's open map,
  2. builds a dependency-free procedural city blockout in that scratch
     level (the Rivendell-style elven city from
     scripts/elven_city_scene.py, executed remotely via the
     execute_unreal_python handler),
  3. spawns an orbit camera actor and walks it around the city on a
  ...
  4. assembles the captured PNG frames into docs/images/demo.gif with
     ffmpeg using a two-pass palettegen for quality at a small size,
  5. on success, attempts a CLEAN editor exit (load an empty level to
     drop dirty state, then a graceful quit) so no dirty `Untitled`
     autosave is left behind.

Clean-exit rationale
--------------------
Earlier revisions built into the editor's open, unsaved world and then
relied on the editor being force-killed externally. That left a dirty
`Untitled` map, so the next editor launch popped a **Restore-Packages**
dialog. This driver now builds into a dedicated `/Game/_McpScratch/`
scratch level and, on success, asks the editor to load an empty level
(dropping dirty state) and quit gracefully. A force-kill is NOT this
script's job, but if a stale Restore-Packages dialog ever appears from a
previous unclean run, answer **"Skip Restore"** — these runs build a
throwaway scratch level, never user content, so there is nothing in the
autosave worth recovering. The specific UE clean-exit APIs used here
(`LevelEditorSubsystem.new_level` / `.load_level`,
`unreal.SystemLibrary.quit_editor`, with `EditorLevelLibrary`
fallbacks) are written defensively/best-effort and are empirically
validated only in the live Stage-C proof run (UE is not running while
this file is authored — treat them as proof-pending until then).
...
# Dedicated throwaway scratch level. The build + capture happen HERE, never
# in the user's open map, so a force-kill (or any unclean exit) cannot leave a
# dirty `Untitled` autosave that triggers a Restore-Packages dialog. See the
# module docstring's "Clean-exit rationale".
DEFAULT_SCRATCH_LEVEL = "/Game/_McpScratch/DemoLevel"
# Success sentinels for the two clean-exit remote snippets (logged via
# unreal.log and read back from the editor log ring buffer — the same
# marker pattern the camera synthetic tools already rely on).
SCRATCH_LEVEL_OK = "UCMCP_SCRATCH_LEVEL_OK"
CLEAN_EXIT_READY = "UCMCP_CLEAN_EXIT_READY"
...
# ---------------------------------------------------------------------------
# Scratch-level + clean-exit helpers.
#
# These are the ONLY behavioural change vs. the original capture pipeline:
# the build is retargeted to a dedicated /Game/_McpScratch/ level, and on
# success the editor is asked to drop dirty state + quit gracefully. The
# orbit/capture/ffmpeg logic below is untouched.
#
# Every UE API used here is wrapped defensively: each call tries the modern
# LevelEditorSubsystem first and falls back to EditorLevelLibrary (the exact
# proven fallback chain scripts/elven_city_hifi.py already uses for
# save_current_level). A failure logs a sentinel-absent marker rather than
# raising — clean exit is best-effort and must never fail the (already
# successful) capture. NOTE: these clean-exit code paths are empirically
# validated only in the live Stage-C proof run; UE is not running while this
# file is authored, so treat them as proof-pending until that run.
# ---------------------------------------------------------------------------

def _ensure_scratch_level(host: str, port: int, scratch_level: str,
                          request_id: int) -> None:
    """Create (or load) the dedicated scratch level so the build never
    dirties the user's open map. Best-effort: a brand-new empty level is
    preferred; loading an existing one is the fallback. Raises CaptureError
    only if BOTH the transport fails AND nothing usable happened — a missing
    success sentinel with an otherwise-ok call is downgraded to a warning so
    a slightly-different UE build can't block the demo."""
    # new_level() makes a fresh empty map at the path (creating the package
...
'''
    ...
    if f"{SCRATCH_LEVEL_OK} 1" in blob:
        ...
        # The editor answered but could not make/load the level. The build
        # would then land in the open map — exactly what we are avoiding —
        # so this IS fatal (unlike a merely-absent sentinel).
        raise CaptureError(
            f"[scratch-level] editor could not create or load "
            f"{scratch_level} (sentinel reported failure). Refusing to "
            f"build into the user's open map. result={result}"
        )
    else:
        # Sentinel not echoed back (handler output shape varies across
        # builds). Don't hard-fail on output-shape alone; warn and proceed —
        # the post-build actor-count sanity still guards a real failure.
        print(f"  (note: '{SCRATCH_LEVEL_OK}' sentinel not echoed; relying "
              f"on the post-build actor-count check. raw keys: "
              f"{sorted(result.keys())})")


def _attempt_clean_exit(host: str, port: int, request_id: int) -> None:
    """Best-effort clean editor shutdown AFTER a successful capture:

      1. load an empty level so the dirty scratch world is dropped (no
         `Untitled` autosave → no Restore-Packages dialog next launch),
      2. ask the editor to quit gracefully via
         unreal.SystemLibrary.quit_editor() (AutomationLibrary /
         EditorLevelLibrary fallbacks).

    This NEVER raises: the GIF is already on disk and the run is a success;
    a shutdown hiccup must not flip the exit code. A force-kill is not this
    script's responsibility — it is the LAST resort for whatever launched
    the editor, applied only if this graceful path did not terminate it
    within that caller's own timeout (see module docstring)."""
    snippet = f'''
import unreal
# (a) Drop dirty state: load a blank template level so the dirty scratch
#     world is no longer the current/unsaved map. new_level on an empty
#     path == a fresh untitled-but-clean world; an explicit empty-level
#     load is the fallback.
try:
    _les = unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
    try:
        _les.new_level("/Temp/_UcmcpEmpty")
    except Exception:
        try:
            _les.load_level("/Engine/Maps/Templates/OpenWorld")
        except Exception:
            pass
except Exception:
    try:
        unreal.EditorLevelLibrary.new_level("/Temp/_UcmcpEmpty")
    except Exception:
        pass
unreal.log("{CLEAN_EXIT_READY} 1")
# (b) Graceful quit. quit_editor() is the documented editor-shutdown entry
#     point; guard it so a missing/renamed API still logs the readiness
#     marker above (the caller can then fall back to its own force-kill).
try:
    unreal.SystemLibrary.quit_editor()
except Exception:
    try:
        unreal.AutomationLibrary.quit_editor()
    except Exception:
        try:
            unreal.EditorLevelLibrary.editor_request_exit()  # noqa
        except Exception:
            pass
'''
    # A graceful quit may tear the socket down mid-reply; a transport error
    # here is EXPECTED and benign (the editor is exiting, which is the goal).
    try:
        resp = call(host, port, "execute_unreal_python",
                    {"code": snippet}, request_id=request_id, timeout=30.0)
    except Exception as e:  # pragma: no cover - defensive belt
        print(f"  clean-exit: request raised {e!r} (editor likely already "
              f"quitting — benign).")
        return
    if "_error" in resp:
        print(f"  clean-exit: transport closed during graceful quit "
              f"({resp['_error']}) — expected if the editor is exiting.")
        return
    try:
        result = unwrap(resp, "execute_unreal_python[clean-exit]")
        blob = json.dumps(result, default=str)
        if CLEAN_EXIT_READY in blob:
            print("  clean-exit: empty level loaded + graceful quit "
                  "requested.")
        else:
            print(f"  clean-exit: quit requested (readiness sentinel not "
                  f"echoed; raw keys: {sorted(result.keys())}).")
    except CaptureError as e:
        # Even an unparseable reply here is non-fatal: the run already
        # succeeded and the quit was requested on a best-effort basis.
        print(f"  clean-exit: graceful-quit reply not parseable "
              f"({e}) — non-fatal, capture already succeeded.")
...
def main() -> None:
    ...
    ap.add_argument("--skip-build", dest="skip_build", action="store_true",
                    help=("skip the procedural scene build and capture the "
                          "current level as-is"))
    ap.add_argument("--scratch-level", dest="scratch_level",
                    default=DEFAULT_SCRATCH_LEVEL,
                    help=(f"dedicated throwaway level the build is targeted "
                          f"into so the user's open map is never dirtied "
                          f"(default {DEFAULT_SCRATCH_LEVEL}). Skipped when "
                          f"--skip-build is set (then the current level is "
                          f"captured as-is)."))
    ap.add_argument("--no-clean-exit", dest="no_clean_exit",
                    action="store_true",
                    help=("do NOT attempt the graceful editor quit after a "
                          "successful capture (leave the editor running). "
                          "By default the editor is asked to drop dirty "
                          "state and quit so no dirty Untitled autosave is "
                          "left behind."))
    args = ap.parse_args()
    ...
    print(f"  scratch lvl: "
          f"{'(skipped — --skip-build)' if args.skip_build else args.scratch_level}")
    print(f"  clean exit : "
          f"{'NO (--no-clean-exit)' if args.no_clean_exit else 'yes (graceful quit)'}")
    ...
    try:
        ...
        # --- (a.5) retarget to the dedicated scratch level -------------
        # CRITICAL for clean exit: switch to a throwaway /Game/_McpScratch/
        # level BEFORE building, so the scene script's own
        # save_current_level() saves THAT level — never the user's open
        # map. Without this the build dirties `Untitled` and an unclean
        # exit pops a Restore-Packages dialog. Skipped under --skip-build
        # (the operator explicitly asked to capture the current level).
        if not args.skip_build:
            print(f"\n[1.5/5] retargeting to scratch level "
                  f"{args.scratch_level} (so the build never dirties the "
                  f"open map) ...")
            _ensure_scratch_level(host, port, args.scratch_level,
                                  request_id=2)
        ...
        if not args.skip_build:
            ...
            resp = call(host, port, "execute_unreal_python",
                        {"code": code}, request_id=20, timeout=300.0)
    ...
    # --- clean editor exit (success path only) -------------------------
    # The GIF is on disk and the run succeeded. Ask the editor to drop the
    # dirty scratch world and quit gracefully so the next launch does NOT
    # pop a Restore-Packages dialog. This is best-effort and never flips
    # the (already PASS) exit code; the graceful quit may also close the
    # socket mid-reply, which is expected and handled inside the helper.
    # A force-kill remains the LAST resort for whatever launched the
    # editor (applied only if this graceful path did not terminate it
    # within that caller's timeout) — see the module docstring.
    if args.no_clean_exit:
        print("\n  --no-clean-exit: leaving the editor running "
              "(scratch level may remain dirty until closed manually).")
    else:
        print("\n  clean exit: dropping dirty scratch state + requesting "
              "a graceful editor quit ...")
        _attempt_clean_exit(host, port, request_id=9000)
