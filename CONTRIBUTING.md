...
Please skim this file before opening a PR. Three of the rules are slightly unusual for a standard open-source project; they exist because the codebase has 106 tools total and a non-trivial cross-language split between C++ (UE plugin) and Python (bridge).
...
- `tests/` — pytest suite for the bridge. **No UE required.** 518 pytest cases.
...
**Fixing a bug?** Open an issue first if it isn't obvious. Tests-first when reasonable. The bridge has 518 pytest cases — if the bug is in the bridge, write a failing test, then fix it.

**Improving docs?** Welcome. Just keep the canonical-count phrasing in digit form (`106 tools`, not `One hundred five tools`) — `scripts/drift_sweep.py` enforces digit counts, and English-word counts silently survive the sweep.
...
- **Synthetic tools must validate `isinstance(args, dict)` early** and return a clean `-32602 invalid_arguments` envelope on mismatch. All 34 synthetic tools do this; the test `test_synthetic_returns_invalid_arguments_for_non_dict_args` locks it.
