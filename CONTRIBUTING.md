...
- `tests/` — pytest suite for the bridge. **No UE required.** 563 pytest cases.
...
**Fixing a bug?** Open an issue first if it isn't obvious. Tests-first when reasonable. The bridge has 563 pytest cases — if the bug is in the bridge, write a failing test, then fix it.
