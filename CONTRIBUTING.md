...
Please skim this file before opening a PR. Three of the rules are slightly unusual for a standard open-source project; they exist because the codebase has 116 tools total and a non-trivial cross-language split between C++ (UE plugin) and Python (bridge).
...
- `tests/` — pytest suite for the bridge. **No UE required.** 541 pytest cases.
...
**Fixing a bug?** Open an issue first if it isn't obvious. Tests-first when reasonable. The bridge has 541 pytest cases — if the bug is in the bridge, write a failing test, then fix it.
...
- **Synthetic tools must validate `isinstance(args, dict)` early** and return a clean `-32602 invalid_arguments` envelope on mismatch. All 35 synthetic tools do this; the test `test_synthetic_returns_invalid_arguments_for_non_dict_args` locks it.
