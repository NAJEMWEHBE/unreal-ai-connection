...
- **HANDOFF.md** — reflects the current state (133 tools total — 98 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **563 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **133 tools shipped** (98 UE C++ handlers + 35 bridge-side synthetic)
