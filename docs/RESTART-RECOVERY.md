...
- **HANDOFF.md** — reflects the current state (130 tools total — 95 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **560 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **130 tools shipped** (95 UE C++ handlers + 35 bridge-side synthetic)
