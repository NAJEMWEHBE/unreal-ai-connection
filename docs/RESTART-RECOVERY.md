...
- **HANDOFF.md** — reflects the current state (121 tools total — 86 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **546 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **121 tools shipped** (86 UE C++ handlers + 35 bridge-side synthetic)
