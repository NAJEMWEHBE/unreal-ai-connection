...
- **HANDOFF.md** — reflects the current state (151 tools total — 114 native C++ handlers + 37 synthetic tools, all directives, all traps)
...
Expected: **645 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **151 tools shipped** (114 UE C++ handlers + 37 bridge-side synthetic)
