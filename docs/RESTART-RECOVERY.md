...
- **HANDOFF.md** — reflects the current state (112 tools total — 77 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **537 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **112 tools shipped** (77 UE C++ handlers + 35 bridge-side synthetic)
