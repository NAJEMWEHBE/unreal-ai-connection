...
- **HANDOFF.md** — reflects the current state (142 tools total — 105 native C++ handlers + 37 synthetic tools, all directives, all traps)
...
Expected: **576 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **142 tools shipped** (105 UE C++ handlers + 37 bridge-side synthetic)
