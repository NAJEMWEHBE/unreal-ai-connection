...
- **HANDOFF.md** — reflects the current state (119 tools total — 84 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **544 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **119 tools shipped** (84 UE C++ handlers + 35 bridge-side synthetic)
