...
- **HANDOFF.md** — reflects the current state (124 tools total — 89 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **549 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **124 tools shipped** (89 UE C++ handlers + 35 bridge-side synthetic)
