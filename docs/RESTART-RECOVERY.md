...
- **HANDOFF.md** — reflects the current state (129 tools total — 94 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **559 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **129 tools shipped** (94 UE C++ handlers + 35 bridge-side synthetic)
