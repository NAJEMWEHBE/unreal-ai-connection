...
- **HANDOFF.md** — reflects the current state (106 tools total — 72 native C++ handlers + 34 synthetic tools, all directives, all traps)
...
Expected: **521 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **106 tools shipped** (72 UE C++ handlers + 34 bridge-side synthetic)
