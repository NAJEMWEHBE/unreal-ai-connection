...
- **HANDOFF.md** — reflects the current state (109 tools total — 74 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **534 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **109 tools shipped** (74 UE C++ handlers + 35 bridge-side synthetic)
