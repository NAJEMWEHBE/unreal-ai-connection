...
- **HANDOFF.md** — reflects the current state (146 tools total — 109 native C++ handlers + 37 synthetic tools, all directives, all traps)
...
Expected: **580 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **146 tools shipped** (109 UE C++ handlers + 37 bridge-side synthetic)
