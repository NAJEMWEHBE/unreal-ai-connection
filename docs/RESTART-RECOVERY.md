...
- **HANDOFF.md** — reflects the current state (116 tools total — 81 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **541 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **116 tools shipped** (81 UE C++ handlers + 35 bridge-side synthetic)
