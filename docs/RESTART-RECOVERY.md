...
- **HANDOFF.md** — reflects the current state (107 tools total — 72 native C++ handlers + 35 synthetic tools, all directives, all traps)
...
Expected: **529 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **107 tools shipped** (72 UE C++ handlers + 35 bridge-side synthetic)
