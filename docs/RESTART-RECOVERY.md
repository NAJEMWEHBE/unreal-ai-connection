...
- **HANDOFF.md** — reflects the current state (139 tools total — 102 native C++ handlers + 37 synthetic tools, all directives, all traps)
...
Expected: **573 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **139 tools shipped** (102 UE C++ handlers + 37 bridge-side synthetic)
