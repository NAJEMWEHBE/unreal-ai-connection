...
- **HANDOFF.md** — reflects the current state (140 tools total — 103 native C++ handlers + 37 synthetic tools, all directives, all traps)
...
Expected: **574 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **140 tools shipped** (103 UE C++ handlers + 37 bridge-side synthetic)
