...
- **HANDOFF.md** — reflects the current state (143 tools total — 106 native C++ handlers + 37 synthetic tools, all directives, all traps)
...
Expected: **577 pytest cases** passing (verify against the expected-count constants in `tests/conftest.py` and the manifest-parity check in `tests/test_manifest_sync.py`).
...
- **143 tools shipped** (106 UE C++ handlers + 37 bridge-side synthetic)
