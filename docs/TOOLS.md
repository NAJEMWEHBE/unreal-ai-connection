...
> Legacy Phase-H gate (inactive under UE 5.7-only): **Min engine: 5.0** — uses `unreal.get_editor_subsystem` (5.0+). On older engines the bridge returns a structured `unsupported_on_engine_version` error (Phase H gate, `docs/PHASE-H-COMPAT.md`).
...
> Legacy Phase-H gate (inactive under UE 5.7-only): **Min engine: 5.0** — uses 5.0+ `unreal.*` editor-subsystem / render-target-cube APIs. On older engines the bridge returns a structured `unsupported_on_engine_version` error (Phase H gate, `docs/PHASE-H-COMPAT.md`).
...
> Legacy Phase-H gate (inactive under UE 5.7-only): **Min engine: 5.0** — uses `unreal.MovieSceneTimeUnit` and the 5.0+ MovieScene scripting channel APIs. On older engines the bridge returns a structured `unsupported_on_engine_version` error (Phase H gate, `docs/PHASE-H-COMPAT.md`).
