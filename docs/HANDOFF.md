...
> Earlier closing notes (1st through 30th, sessions 2026-05-09 through 2026-05-16) are archived to [`docs/HANDOFF-archive.md`](HANDOFF-archive.md). This active file keeps the latest three consecutive notes (31st-33rd) for quick pickup.
...
**UE 5.7 is the only supported version** (ADR-0001, 2026-05-18). Cross-engine compatibility scaffolding (Phase-H shims, T1/T2/T3 buckets, per-bucket `.uplugin` generator) is FROZEN and left inert in the codebase — historical reference only, not maintained. Do not re-expand cross-version support without an explicit user request.
...
> **Note:** Consecutive closing notes 1 through 30 (sessions 2026-05-09 through 2026-05-16) are archived in [`HANDOFF-archive.md`](HANDOFF-archive.md). Only the latest three (31st-33rd) are kept active here.
...
**Carried forward / next actions (scope now UE 5.7-only — ADR-0001, 2026-05-18):**
- **Feature work targets UE 5.7 APIs only.** New handlers follow the one-`.cpp`-plus-one-registration-line pattern against UE 5.7 source; no version-gated branch, no `UCMCPCompat.h` shim, no per-bucket `.uplugin` step.
...
- **Optional later:** Phase H scaffold archival (move the inert cross-engine machinery out of the active tree) — low priority, only if it reduces noise; the scaffold is reversible-by-design and harmless left in place per ADR-0001.
- **RETIRED under ADR-0001 (no longer carried):** UE 4.27 (T3) host-verification — dropped (never host-verified; needs a real 4.27 engine that is now out of scope); the per-bucket `.uplugin` generator + multi-version drop-in/build recipes — retired (UE 5.7-only releases); "Optional 5.1 polish" — dropped (T2/UE 5.1 was historically host-verified in the 31st-note window and is kept as a record only); Phase H remaining-clusters extension — frozen, not maintained.
...
**Thirty-second consecutive closing-note.** Session 2026-05-17: branch `verify/wave-a-host-rebuild-ue57` (base `b1c065c`, PR #219). The perpetual next step is **CLOSED** — host UE 5.7 (T1) cold rebuild compiled in the 9 formerly-inert C++ handlers (Wave A 5 + Wave A.5 2 + `render_camera_to_png`) and proved them live. Pre-audit DONE and clean (all 9 LIKELY-COMPILES vs actual `F:\UE_5.7` source, no Build.cs gaps, registration 9/9; NVIDIA API answers wrong and discarded per rule #1 — audit correctly predicted first-try success). MSVC pin flipped 14.34→14.44 for the 5.7 build (maintainer-applied; classifier gated the machine-global file) — **RESTORE-to-14.34 completed this session (2026-05-18)** (UE ≤5.3 needs it; `BuildConfiguration.xml` line 4 back to 14.34.31933). robocopy dev→host sync done (host plugin is a plain copy, not a junction). **Host PROVEN:** `Build.bat HDMediaVirtualStudioEditor` → `Result: Succeeded` (~46s, MSVC 14.44.35207); UE 5.7.4 bound `127.0.0.1:18888`; **72** handlers registered (was 71 pre-#214); `examples\smoke_test.py` EXIT 0 (11/11 sections); `examples\verify_wave_a.py` EXIT 0 — 8 C++ handlers PASS with **zero `-32601`** (inert regression CLEARED), `render_camera_to_png` wrote a 2,059,202-byte PNG (29th-note headless-capture root cause closed); `bulk_inspect_assets` SKIP = bridge-only-by-design, not a regression. UE 5.7 (T1) is now host-COMPILE-AND-RUNTIME-verified; 5.1 (T2) verified prior; 4.27 (T3) source-only. Rotated the 29th consecutive closing-note to `HANDOFF-archive.md` (active file keeps 30th–32nd; archive blurb brought current 1–29 / 30–32, correcting a pre-existing 2-cycle lag). Stale doc-counts scrubbed (71→72, 104→105, manifest/synthetic/TOOLS/archive); disproven `UNVERIFIED 5.7` comments cleared in `Handler_RenderCameraToPng.cpp`. Editor left running (PID 27988) — deliberate exception to "open, verify, close": the live socket is held open so the maintainer can run the merge-safety bot-gate against the same proven build without a cold relaunch. **Next-operator cleanup:** once PR #220 is merged (or if resuming cold), terminate it on the host with `Stop-Process -Id 27988 -Force` (or close the editor window) before the next build cycle. Tool count: **105** (72 C++ + 33 synthetic). pytest: **498**. Standing rules: **5 (unchanged; prior notes' "6" was a miscount — the operative section lists exactly 5)**. Cadence intact.

---

## Session 2026-05-18 (scope locked to UE 5.7-only — ADR-0001; cross-engine compat scaffold FROZEN; docs-only)

The project scope was narrowed by user decision: UE 5.7 is now the only supported version. This session is docs/policy-only — no `.cpp/.h/.Build.cs` edits, no rebuild. The previously-host-proven artifacts from the 32nd note are the operative state going in.

**Prior task shipped (from the 32nd-note branch):** the `verify/wave-a-host-rebuild-ue57` work was squash-merged as **PR #220 → `main` (HEAD `5f4f596`)**; the feature branch was deleted; the host toolchain was restored to **MSVC 14.34.31933** (`BuildConfiguration.xml` line 4); the editor (PID 27988) was killed. No code or rebuild in that close-out beyond the merge — the 32nd note's host-PROVEN UE 5.7 (T1) compile+runtime verification stands as the baseline.

**Scope decision (ADR-0001, 2026-05-18):** the user locked support to **UE 5.7 only**. `docs/adr/ADR-0001-ue57-only-freeze-cross-engine-compat.md` was created (Accepted). The Phase H / T1/T2/T3 cross-version certification cadence is **FROZEN and RETIRED**: T2 (UE 5.1) was historically host-verified in the 31st-note window and is kept as a record only; T3 (UE 4.27) was never host-verified and is dropped; the `UCMCPCompat.h` shims, per-handler version gates, and the per-bucket `.uplugin` generator are left **inert in the codebase, reversible-by-design**, not maintained — no `.cpp/.h/.Build.cs` touched (the compat shims stay as-is by decision).

**Docs reconciled this session:** ADR-0001 created; `docs/HANDOFF.md` — operative carry-forwards rewritten to 5.7-only (cross-version items marked RETIRED under ADR-0001), perpetual next-step annotated as RETIRED, new `## Project scope` section inserted after the standing-rules block, this 33rd note appended, 30th consecutive closing-note rotated to `HANDOFF-archive.md` (active file now keeps 31st–33rd), the 4 active/archive blurb counters updated; `README.md` — "Unreal Engine 5" → "5.7", "Unreal Editor 5.7+" → "5.7", Phase-H roadmap paragraph rewritten to FROZEN/superseded; `docs/PHASE-H-COMPAT.md` — FROZEN banner added after the STATUS block (historical content otherwise intact); `UnrealClaudeMCP.uplugin` — stale `Description` counts corrected to 72 native / 33 synthetic / 105 total (`EngineVersion` already `5.7.0`, untouched); `docs/TOOLS.md` — legacy Phase-H synthetic-gate notes prefixed as inactive under UE 5.7-only. Standing rules stay **5** (the new scope is a separate `## Project scope` section, not a 6th rule).

**Carried forward / next actions (UE 5.7-only — ADR-0001):**
- Feature work targets UE 5.7 APIs only — one `.cpp` + one registration line per handler, no version gate / shim / per-bucket variant.
- README demo GIF — unblocked (`render_camera_to_png` host-built per the 32nd note); creative-recording window.
- Optional later: Phase H scaffold archival (move the inert machinery out of the active tree) — low priority, reversible-by-design, harmless left in place.
- **Maintainer-only (unchanged, still live):** merge this branch's PR after the bot-gate; close PR #207 + revoke the `ecc-tools` GitHub App (hostile-bundle foothold); decide PR #205 A/B. Standing carry-forward (5.7-scoped): rename Stage 2 (C++ module identity — host rebuild); Phase J (BTSschool — separate dir); Movie Render Queue synthetic (attended-Codex C++); local OSS LLM daemon empty-list bug (admin shell required).

**Thirty-third consecutive closing-note.** Session 2026-05-18: docs/policy-only — no code or rebuild. Prior task shipped (PR #220 squash-merged → `main` HEAD `5f4f596`; feature branch deleted; toolchain restored to MSVC 14.34.31933; editor PID 27988 killed). User locked scope to **UE 5.7-only** — `docs/adr/ADR-0001-ue57-only-freeze-cross-engine-compat.md` created (Accepted, 2026-05-18). The Phase H / T1/T2/T3 cross-version certification cadence is FROZEN & RETIRED: T2 (UE 5.1) was historically host-verified (31st-note window) and is kept as a record only; T3 (UE 4.27) was never host-verified and is dropped; `UCMCPCompat.h` shims + per-handler version gates + the per-bucket `.uplugin` generator left inert and reversible-by-design, not maintained (no `.cpp/.h/.Build.cs` touched). Docs reconciled: ADR-0001 + HANDOFF (5.7-only carry-forwards, perpetual-next-step retired, new `## Project scope` section, this note, 30th note rotated to archive, 4 counters updated) + README (UE 5.7 wording + Phase-H FROZEN) + PHASE-H-COMPAT (FROZEN banner) + `.uplugin` (Description counts 72/33/105; `EngineVersion` 5.7.0 untouched) + TOOLS.md (legacy Phase-H gate notes prefixed inactive). Rotated the 30th consecutive closing-note to `HANDOFF-archive.md` (active file now keeps 31st–33rd; archive holds 1–30). Tool count: **105** (72 C++ + 33 synthetic). pytest: **498**. Standing rules: **5 (unchanged)**. Cadence intact.
