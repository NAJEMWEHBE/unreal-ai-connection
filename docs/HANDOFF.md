...
> **Note:** Consecutive closing notes 1 through 28 (sessions 2026-05-09 through 2026-05-17) are archived in [`HANDOFF-archive.md`](HANDOFF-archive.md). Only the latest four (29th-32nd) are kept active here.
...
## Session 2026-05-18 (TCP-server shutdown-crash fix — `TickClients()` iterator invalidation, host-build VERIFIED)

Fixed a reproducible TCP-server shutdown crash: `FUCMCPServer::TickClients()` iterator invalidation — UE ensure `"Array has changed during ranged-for iteration!"` firing at `MCPServer.cpp:274`/`327`. Root-caused to **two real, independent defects**, not the originally-suggested ones. **D1 (reentrancy):** a handler dispatched mid-tick via `execute_unreal_python` → `quit_editor` → `FUCMCPServer::Stop()` → `ConnectedClients.Empty()` structurally mutates the array while the ranged-for is still on the stack. **D2 (cross-thread race):** `FTcpListener` runs its own accept thread, so `OnConnectionAccepted` was racing `ConnectedClients`/`ReadStates`/`WriteStates` `.Add` against the game-thread loop with no lock. The originally-proposed removal-only fixes (a)/(b)/(c) would NOT have fixed it — **removal was already correctly deferred** via the local `Dropped` array; the bug is on the *add* and *teardown* sides. **Fix:** a lock-guarded `PendingAccepted` (FCriticalSection) drained at the top of `TickClients` so connects are now game-thread-only and outside the ranged-for, plus a `bTicking`/`bStopRequested` reentrancy guard that defers `Stop()` teardown until the ranged-for unwinds. All UE 5.7 APIs verified against on-disk `F:\UE_5.7` source (`FTcpListener` = `FRunnable` own-thread; `FScopeLock`/`FCriticalSection` includes; `TQueue` SPSC is deprecated so used `FCriticalSection`+`TArray`; the `TARRAY_RANGED_FOR_CHECKS` checked-iterator macro in `Core/Public/Containers/Array.h`). New ARCHITECTURE.md trap added for this. **Proof:** clean host build → `Result=Succeeded`, `MCPServer.cpp` compiled in its own TU, 0 errors / 0 warnings, VS2022 14.44 standard toolchain. On a feature branch; bot-gate pending; not yet merged. Tool count unchanged (105).

**Session harness note (load-bearing for the next agent):** this session's harness PowerShell tool was non-functional (every call `Exit 1`, stdout suppressed) — all shell ran via the Bash tool (native commands only). The system .NET 8 runtime is also missing (only 3.1.29 + 10.0.8 present), so the host build had to be driven through the **engine-bundled** `dotnet` + `UnrealBuildTool.dll` invoked from Bash, not the system SDK. If PowerShell is dead again, do the same.

**Carried forward unchanged (still open):** bot-gate + merge this TCP-fix branch (PRIMARY next-action — verify-then-hand-the-merge-command-over per the merge-classifier constraint). UE 4.27 (T3) still source-only (needs a real 4.27 engine). Maintainer-only (unchanged, still live): close PR #207 + revoke the `ecc-tools` GitHub App (hostile bundle foothold); decide PR #205 A/B. Standing carry-forward: rename Stage 2 (C++ module identity — host rebuild), Phase J (BTSschool — separate dir), README demo GIF, Movie Render Queue synthetic, local OSS LLM daemon empty-list bug.

**Thirty-second consecutive closing-note.** Session 2026-05-18: reproducible TCP-server shutdown crash fixed — `FUCMCPServer::TickClients()` ranged-for iterator invalidation (`MCPServer.cpp:274`/`327`, UE `"Array has changed during ranged-for iteration!"` ensure). Two real defects: D1 reentrant `execute_unreal_python`→`quit_editor`→`Stop()`→`ConnectedClients.Empty()` mutating the array mid ranged-for; D2 `FTcpListener`'s own thread racing `ConnectedClients`/`ReadStates`/`WriteStates` `.Add` with no lock. Removal was ALREADY deferred (`Dropped` array), so the suggested removal-only fixes (a)/(b)/(c) would not have helped. Fix: lock-guarded `PendingAccepted` drained at top of `TickClients` (connects now game-thread-only, outside the ranged-for) + `bTicking`/`bStopRequested` reentrancy guard deferring `Stop()` teardown until the ranged-for unwinds. UE 5.7 APIs verified vs `F:\UE_5.7` source. Clean host build: `Result=Succeeded`, 0 errors / 0 warnings, VS2022 14.44. ARCHITECTURE.md trap added. On a feature branch; bot-gate pending. Harness PowerShell tool was dead this session (all shell via Bash; engine-bundled dotnet+UBT used since system .NET 8 absent). No rotation this session (docs-only fix-note pass — active file keeps 29th–32nd; next agent rotates the 29th if/when the cadence cap is enforced). Tool count: **105** (72 C++ + 33 synthetic). pytest: **498**. Cadence intact.
