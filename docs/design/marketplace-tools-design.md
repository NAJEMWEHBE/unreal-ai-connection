...
Two new bridge-side synthetic MCP tools — `marketplace_search` and `marketplace_import` — that let an MCP client (Claude Code, Codex CLI, Cursor, Gemini CLI, etc.) browse public CC0 / CC-BY asset libraries and pull textures + HDRIs straight into a UE 5.7 project as `UTexture2D` assets, without leaving the editor session. v1 targets Polyhaven (uniformly CC0) and Sketchfab (CC0 + CC-BY filtered server-side). 3D model (.fbx/.glb) import is explicitly **out of scope for this PR** — the native side has no mesh-import wrapper yet, so models land in PR #3. Both tools are stdlib-only (`urllib.request` + `urllib.parse`) and add no Python runtime dependency.

> **Prior-art note.** The bridge already ships a Polyhaven + AmbientCG implementation (search both `synthetic_marketplace_search` and `synthetic_marketplace_import` in `bridge/unreal_claude_mcp_bridge.py`, ~line 5040). This design supersedes the AmbientCG path with Sketchfab — AmbientCG's download artefacts are zipped multi-map archives that v1 cannot unpack cleanly. The maintainer should either (a) replace the AmbientCG branch with the Sketchfab branch below, or (b) keep AmbientCG behind a `source=ambientcg` flag and add `source=sketchfab` alongside. The two backends are independent.
