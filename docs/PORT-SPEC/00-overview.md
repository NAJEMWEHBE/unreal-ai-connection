...
**Registration.** **C++ toolsets do NOT auto-register** (proven live, K2 build session 2026-07-18 — earlier "auto-discovers subclasses via reflection" claim was wrong for C++). Every C++ toolset module must call `UToolsetRegistry::RegisterToolsetClass(UMyToolset::StaticClass())` in `StartupModule()` and `UnregisterToolsetClass()` in `ShutdownModule()` — exactly what every Epic C++ toolset module does (see `Engine/Plugins/Experimental/Toolsets/ConfigSettingsToolset/Source/ConfigSettingsToolset/Private/ConfigSettingsToolsetModule.cpp`). Auto-discovery via reflection covers **Python toolsets only**. Missed-registration symptom: DLL loads clean, zero log lines, toolset absent from `list_toolsets`. `ModelContextProtocol.RefreshTools` re-polls. Live Coding handles body edits; **new `UFUNCTION`s need an editor restart**. Cooked builds: no discovery, editor-only adapter — this plugin is editor-only by design.

**Modal-dialog deadlock rule** (proven live, same session). Tool calls run serially on the game thread; any editor API that reaches `ShowModal()` hangs the MCP server until a human clicks (hit live: `FBlueprintEditorUtils::ChangeMemberVariableType` pops `FSuppressableWarningDialog` when the variable is in use). Before using any `FBlueprintEditorUtils`/editor-utility call in a tool body, grep its .cpp for `ShowModal|FMessageDialog|OpenMsgDlg`; if present, reimplement the core mutation headless. Full gotcha list from that session: `F:\ai\brain\concepts\ue58-toolset-build-gotchas.md`.
...
1. Enable plugins `ModelContextProtocol`, `ToolsetRegistry`, `AllToolsets` (+ ours). Reusable smoke project: `F:\ai\claude-work\McpSmoke\`; working smoke client + driver: `F:\ai\claude-work\McpSmoke\SmokeTools\` (`mcp_client.py`, `smoke_run.py` — handle the SSE quirks below, use these instead of writing a new client).
