...
UnrealAIConnection depends on subsystems introduced in 4.27 (UImportSubsystem, and the predecessor APIs to ULevelEditorSubsystem).
...
> `DMXProtocol` plugins or `UnrealAIConnection` fails to load. **Toolchain note:**
...
`UnrealAIConnection/UnrealAIConnection.uplugin` and emits three copies into
`dist/uplugin-variants/`, each differing from the canonical file on
**exactly one line** (the `EngineVersion` load-gate):

    dist/uplugin-variants/UnrealAIConnection-T1.uplugin -> "EngineVersion": "5.4.0"   # UE 5.4 - 5.8
    dist/uplugin-variants/UnrealAIConnection-T2.uplugin -> "EngineVersion": "5.0.0"   # UE 5.0 - 5.3
    dist/uplugin-variants/UnrealAIConnection-T3.uplugin -> "EngineVersion": "4.27.0"  # UE 4.27
...
Then, per bucket: copy the matching variant over `UnrealAIConnection.uplugin`
...
Current state: UnrealAIConnection/UnrealAIConnection.uplugin line 5 has "EngineVersion": "5.7.0".
...
| Save-delegate (PostSaveWorldWithContext / PostSaveWorld) | **shim-wired (unverified-compile)** -- `UCMCP_POST_SAVE_WORLD_DELEGATE`/`UCMCP_POST_SAVE_CONTEXT_TYPE` wired in `UnrealAIConnectionModule.cpp` (register + unregister); `ObjectSaveContext.h` include gated `#if UCMCP_ENGINE_AT_LEAST(5,0)`. **UNVERIFIED**: 4.27 `PostSaveWorld(uint32,UWorld*,bool)` param list + `UCMCP_POST_SAVE_CONTEXT_TYPE==bool` placeholder |
...
### Intended matrix structure (documentation only -- not a real workflow)
    ...
    steps:
      ...
      - build: RunUAT BuildPlugin -Plugin=UnrealAIConnection.uplugin -TargetPlatforms=Win64
...
1. UnrealAIConnection/Resources/mcp_manifest.json (the tools array)
