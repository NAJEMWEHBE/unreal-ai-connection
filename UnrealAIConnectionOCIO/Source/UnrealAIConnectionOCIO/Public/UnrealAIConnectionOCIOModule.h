...
/**
 * Optional companion module. On load it registers the OCIO MCP handler into the
 * core UnrealAIConnection handler registry; on unload it removes it. The core
 * plugin has no OCIO dependency, so it loads on hosts without the OpenColorIO
 * plugin; enabling this companion is what brings the inspect_ocio_config tool online.
 */
class FUnrealAIConnectionOCIOModule : public IModuleInterface
{
public:
